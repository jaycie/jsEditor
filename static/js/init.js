...
$(function(){
	...
	$('#createPageNow').on('click',function(){
		...
		if(packetSetting && packetSetting.totalMoney>0){ //红包数据
			_js +='<script src="'+siteConfig.url.editor+'/static/js/jquery-1.10.2.min.js"></script>' +
				  '<script>$(function(){if(location.href.indexOf("grabBonus.html")>0){function getUrl(param){var reg=new RegExp("(^|&)"+param+"=([^&]*)(&|$)");var r=window.location.search.substr(1).match(reg);if(r!=null)return unescape(r[2]);return null}$.getJSON(editConfig.packetSetting.openUrl+"?token="+$("#testIframe",parent.document).data("token")+"&hongbaoId="+getUrl("hongbaoId")+"&hongbaoKey="+getUrl("hongbaoKey")+"&callback=?",function(data){$(".locationGrabBonus .data-get").html(data.hongBao.hongbao/100);$(".locationGrabBonus .data-share").html(data.hongBao.leftHongbaoPoint/100)})}else{$(".grabBonus").on("click",function(){$.getJSON(editConfig.packetSetting.getUrl+"?token="+$("#testIframe",parent.document).data("token")+"&activeId="+editConfig.pInfo.aId+"&callback=?",function(data){if(data.status==="success"){window.location.href="grabBonus.html?token="+data.token+"&hongbaoId="+data.hongbaoInfo.hongbaoId+"&hongbaoKey="+data.hongbaoInfo.hongbaoKey}else if(data.code=="2102"){alert("红包已领过")}else if(data.code=="2101"){alert("红包已领完")}})})}})</script>';
