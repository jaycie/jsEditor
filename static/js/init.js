...
$(function(){
	...
	$('#createPageNow').on('click',function(){
		var cPConfig={
			url:　$dragBox.css('backgroundImage').replace('url(','').replace(')','').replace(/\'/g,'').replace(/\"/g,''),
			top: $dragBox.css('top'),
			...
			_html= '<!DOCTYPE html><html><head><meta charset="utf-8">' +
				...
				'<style>body{margin: 0;padding: 0;max-width: 100%;overflow: hidden;}body img{max-width:100%;max-height:100%}.drag-bg{position:absolute;z-index:-1;left:0;top:'+_img.top+';width:'+cPConfig.width+';height:'+cPConfig.height+';}'+
